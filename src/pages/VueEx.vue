<template class="q-pa-md q-gutter-md">
  ...
  <q-card-section :class="[activeClass,errorClass]">changecolor</q-card-section>
  ...
  <q-card-section v-else-if="type === 'C'"> C </q-card-section>
  <q-card-section v-else> Not A/B/C </q-card-section>
  <div class="q-pa-md row items-start q-gutter-md">  <!--template에서 v-if -->
...
v-for="(value, key, index) in myObject" 를 사용할수도있음-->
<div class="q-pa-md row items-start">
...
export default {
  ...
  watch: {
    ...
    key: { //콜백 실행 타이밍
      handler() {},
      flush: 'post' //콜백을 버퍼링, 마지막에 한번만 호출
    },
...
reverseNumbers() {
  ...
  return [...numbers].reverse();
    ...
    calculateDate() {
      ...
      return this.str; //binding되는 변수 필요
      // const timeStamp = Date.now();
